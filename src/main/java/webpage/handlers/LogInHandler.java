    ...
    public void handle() {
        path("/login", () -> {
            ...
            post("", "application/json", (req, res) -> {
                ...
                if (!(logInRequest.getUsername() != null && logInRequest.getPassword() != null)) return returnMessage(res, 401, "You need to fill all the fields");
                Optional<User> user = findUserByUsername(logInRequest.getUsername());
                if (user.isEmpty()) return returnMessage(res, 406, "Incorrect username or password");
